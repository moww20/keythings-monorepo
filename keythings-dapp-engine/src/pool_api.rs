...
use crate::pool::{PoolManager, PoolType, PoolError};
...
async fn verify_user_can_deposit(
    ...
    keeta_client.verify_acl(wallet_address, pool_storage_account, "STORAGE_DEPOSIT").await
}
...
async fn verify_storage_can_hold(
    ...
    keeta_client.verify_acl(storage_account, token, "STORAGE_CAN_HOLD").await
}
    ...
    let pool_infos: Vec<PoolInfo> = pools
        ...
        .map(|pool| PoolInfo {
            ...
            pool_type: match pool.pool_type {
                ...
                PoolType::StableSwap { amplification } => format!("stable_swap(A={})", amplification),
                PoolType::Weighted { weight_a, weight_b } => format!("weighted({}/{})", weight_a, weight_b),
            },
...
pub async fn create_pool(
    ...
    let pool_type = match body.pool_type.as_deref() {
        ...
        Some("weighted") => PoolType::Weighted { weight_a: 80, weight_b: 20 },
    ...
    if available_b == 0.0 {
        state.ledger.credit(wallet_address, &body.token_b, 10_000_000.0);
    }
    
    if !state.ledger.reserve(wallet_address, &body.token_b, amount_b as f64) {
        state.ledger.release(wallet_address, &body.token_a, amount_a as f64);
        return HttpResponse::BadRequest().json(serde_json::json!({
    ...
    let storage_account = format!("keeta:storage:pool:{}:{}:{}", pool_id_str, body.token_a, body.token_b);
    ...
    let pool_id = match state.pool_manager.create_pool(
        ...
        Err(e) => {
            ...
            state.ledger.release(wallet_address, &body.token_a, amount_a as f64);
            state.ledger.release(wallet_address, &body.token_b, amount_b as f64);
    ...
    if let Err(e) = state.pool_manager.update_storage_account(&pool_id, storage_account.to_string()) {
    ...
    state.ledger.debit_total(wallet_address, &body.token_a, amount_a as f64);
    state.ledger.debit_total(wallet_address, &body.token_b, amount_b as f64);
    
    // STEP 8: Credit LP tokens to user
    let pool = state.pool_manager.get_pool(&pool_id).unwrap();
    state.ledger.credit(wallet_address, &pool.lp_token, pool.total_lp_supply as f64);
...
pub async fn add_liquidity(
    ...
    log::info!(
        ...
        wallet_address, body.pool_id, amount_a, amount_b
    );
    
    // STEP 1: Reserve balances
    if !state.ledger.reserve(wallet_address, &pool.token_a, amount_a as f64) {
        return HttpResponse::BadRequest().json(serde_json::json!({
            "error": "Insufficient balance for token A"
        }));
    }
    
    if !state.ledger.reserve(wallet_address, &pool.token_b, amount_b as f64) {
        state.ledger.release(wallet_address, &pool.token_a, amount_a as f64);
        return HttpResponse::BadRequest().json(serde_json::json!({
    ...
    state.ledger.debit_total(wallet_address, &pool.token_a, amount_a as f64);
    state.ledger.debit_total(wallet_address, &pool.token_b, amount_b as f64);
    state.ledger.credit(wallet_address, &pool.lp_token, lp_tokens as f64);
...
pub async fn remove_liquidity(
    ...
    log::info!(
        ...
        wallet_address, body.pool_id, lp_tokens
    );
    
    // STEP 1: Check user has enough LP tokens
    let (_, total_lp) = state.ledger.internal_balance(wallet_address, &pool.lp_token);
    ...
    state.ledger.debit_total(wallet_address, &pool.lp_token, lp_tokens as f64);
    ...
    state.ledger.credit(wallet_address, &pool.token_a, amount_a as f64);
    state.ledger.credit(wallet_address, &pool.token_b, amount_b as f64);
    
    log::info!(
        "[pool] Liquidity removed: {} {} + {} {}",
        amount_a, pool.token_a,
        amount_b, pool.token_b
    );
    ...
    match pool.get_amount_out(amount_in, &body.token_in) {
        Ok(amount_out) => {
            ...
            HttpResponse::Ok().json(SwapResponse {
                ...
                execution_price: format!("{:.6}", execution_price),
            })
        }
...
pub async fn notify_pool_created(
    ...
    if let Err(e) = state.pool_manager.update_storage_account(&pool_id, body.storage_account.clone()) {
