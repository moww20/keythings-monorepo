    ...
    <main className="relative overflow-hidden">
      ...
      <section className="relative z-10 pt-28 pb-24">
        <div className="mx-auto flex max-w-7xl flex-col gap-16 px-6 lg:flex-row lg:items-center">
          <div className="flex-1 space-y-8">
            <span className="inline-flex w-fit items-center gap-2 rounded-full border border-hairline bg-surface px-4 py-1 text-xs uppercase tracking-[0.2em] text-subtle">
              Chrome Extension
              <span className="h-1.5 w-1.5 rounded-full bg-[color:var(--foreground-subtle)]" />
              Keeta Optimized
            </span>
            ...
            <div className="grid max-w-xl gap-6 sm:grid-cols-3">
              {featureHighlights.map((feature) => (
                <div key={feature.title} className="rounded-2xl border border-hairline bg-surface p-5 text-sm text-subtle shadow-[0_16px_40px_rgba(8,10,14,0.35)]">
          ...
          <div className="flex flex-1 justify-center lg:justify-end">
            <div className="relative w-full max-w-[420px]">
              <div className="absolute -inset-6 rounded-[36px] bg-gradient-to-br from-[color:color-mix(in_oklab,var(--foreground)_14%,transparent)] via-transparent to-[color:color-mix(in_oklab,var(--foreground)_8%,transparent)] blur-2xl" />
              <div className="glass relative overflow-hidden rounded-[30px] border border-hairline p-6 shadow-[0_35px_85px_rgba(0,0,0,0.55)]">
                <div className="absolute inset-x-12 top-8 h-48 rounded-full bg-gradient-to-b from-[color:color-mix(in_oklab,var(--foreground)_22%,transparent)] via-transparent to-transparent blur-2xl" />
                <Image
      ...
      <section className="relative z-10 pb-24">
        <div className="mx-auto max-w-6xl px-6">
          <div className="mb-12 flex flex-col gap-3 text-center">
            <span className="text-xs uppercase tracking-[0.35em] text-faint">Workflow</span>
            <h2 className="text-3xl font-semibold text-foreground md:text-4xl">A secure rhythm from unlock to signature</h2>
            <p className="mx-auto max-w-3xl text-base text-subtle">
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            {workflow.map((step) => (
              <div
                key={step.title}
                className="group relative flex flex-col gap-5 rounded-3xl border border-hairline bg-[color:color-mix(in_oklab,var(--foreground)_3.5%,transparent)] p-6 shadow-[0_20px_60px_rgba(6,7,10,0.45)] transition hover:border-[color:color-mix(in_oklab,var(--foreground)_24%,transparent)] hover:bg-[color:color-mix(in_oklab,var(--foreground)_6%,transparent)]"
              >
                <div className="rounded-2xl border border-hairline bg-gradient-to-br from-[color:color-mix(in_oklab,var(--foreground)_16%,transparent)] via-transparent to-transparent p-4">
                  <Image
                    src={step.image.src}
                    width={320}
                    height={560}
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div className="space-y-6">
              <span className="text-xs uppercase tracking-[0.35em] text-faint">Why teams choose Keythings</span>
              ...
              <dl className="grid gap-6 sm:grid-cols-2">
                <div className="rounded-2xl border border-hairline bg-surface p-5 shadow-[0_18px_50px_rgba(5,6,9,0.45)]">
                  <dt className="text-sm font-semibold text-foreground">Security-first approach</dt>
                  <dd className="text-sm text-muted">
                    Layered defenses keep access private, alert you to risks, and make every signature feel trustworthy.
                  </dd>
                </div>
                <div className="rounded-2xl border border-hairline bg-surface p-5 shadow-[0_18px_50px_rgba(5,6,9,0.45)]">
                  <dt className="text-sm font-semibold text-foreground">Built for momentum</dt>
                  <dd className="text-sm text-muted">
              ...
              <div className="flex flex-wrap gap-4">
                <div className="rounded-full border border-hairline bg-surface px-4 py-2 text-xs uppercase tracking-[0.25em] text-muted">
                  Multi-network ready
                </div>
                <div className="rounded-full border border-hairline bg-surface px-4 py-2 text-xs uppercase tracking-[0.25em] text-muted">
            ...
            <div className="grid gap-6">
              {capabilityCards.map((capability) => (
                <div key={capability.title} className="glass relative overflow-hidden rounded-3xl border border-hairline p-6 shadow-[0_30px_70px_rgba(5,6,11,0.55)]">
                  <div className="absolute inset-x-8 top-6 h-32 rounded-full bg-[color:color-mix(in_oklab,var(--foreground)_18%,transparent)] blur-2xl" />
                  <div className="relative grid gap-4 lg:grid-cols-[1fr_1.2fr] lg:items-center">
                    <div className="space-y-2">
                      <h3 className="text-lg font-semibold text-foreground">{capability.title}</h3>
                      <p className="text-sm leading-relaxed text-subtle">{capability.description}</p>
                    </div>
      ...
      <section className="relative z-10 pb-32">
        <div className="mx-auto max-w-5xl px-6">
          <div className="glass relative overflow-hidden rounded-3xl border border-hairline p-10 text-center shadow-[0_30px_80px_rgba(5,6,11,0.55)]">
            <div className="absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b from-[color:color-mix(in_oklab,var(--foreground)_20%,transparent)] via-transparent to-transparent" />
            <div className="relative space-y-6">
              <span className="text-xs uppercase tracking-[0.35em] text-faint">Ready to deploy</span>
              <h2 className="text-3xl font-semibold text-foreground md:text-4xl">Put the Keeta Network a click away</h2>
              <p className="mx-auto max-w-3xl text-base text-subtle">
